#first I want to call libraries using Dick's function
PackageDependency()
#loading libraries
library(here)
library(epitools)
...
#I want to call in my datasets (UseOfForce).
...
UOF_All_FixLevels <- UOF_All_FixLevels %>%
  mutate(`SubjectRace` = case_when(
    str_detect(`SubjectRace`, "Black") ~ "Black",
    str_detect(`SubjectRace`, "White") ~ "White",
    str_detect(`SubjectRace`, "Indian") ~ "Native",
    str_detect(`SubjectRace`, "Hispanic") ~ "Hispanic",
    str_detect(`SubjectRace`, "Asian") ~ "Asian",
    str_detect(`SubjectRace`, "Hawaiian") ~ "Pacific Islander",
    TRUE ~ `SubjectRace`
  ))
#fix NAs
UOF_All_FixLevels$SubjectRace=gsub(".*NA.*", NA, UOF_All_FixLevels$SubjectRace)

UOF_All_FixLevels$wnw<-UOF_All_FixLevels$SubjectRace
UOF_All_FixLevels <- UOF_All_FixLevels %>%
...
OR_Function(UOF_split$White$`Binning Number of Officers`,UOF_split$White$`Lethal.vs.Non-lethal.Weapon`,UOF_split$White$`Weapon.vs.No Weapon`)

ggplot(UOF_split$Nonwhite,
